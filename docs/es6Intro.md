...
Support for promises is a very nice addition to the language. The example below creates a promise that will call res (resolve) after a second. Then immediately, after that, it will call getCustomerDetails.

Note how I am using the new format of defining functions:

```javascript
const name = (params) -> function body...
```
...
// initialise ourselves.. then get customer details
initialise().then(getCustomerDetails);
```

## Async/Await

Async/Await offer us a cleaner way to use promises. Let's look at an example. We will call a function that takes 5 seconds to execute, then pass the returned value of that in a call to a subsequent function.
...
(async function() {
  async function doStuff() {
    return new Promise((resolve) =>
      setTimeout(() => resolve('data from response is -> ok'), 2500),
    );
  }
  console.time('await');
